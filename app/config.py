...
#   값을 주면 "그 초 동안 한 글자도 안 올 때" 만 끊습니다 — 답이 계속 오는 동안은 총 시간이 길어도 끊지 않습니다.
REQUEST_TIMEOUT = int(os.environ.get("PM_TIMEOUT", "0"))
...
# '묵시적 표현' 으로 보는 분류 — 기술 수단이 문장에 없고 존재만 시사되는 유형.
# 모델 답과 규칙 사전 결과 모두 이 표에서 정합니다 (모델에게 따로 묻지 않음 → 답이 짧아져 빨라짐).
IMPLICIT_CATEGORIES = frozenset({
    "효과만기재", "독자성주장", "최적화·조건확립", "문제해결", "비교우위", "블랙박스용어",
})
