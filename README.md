...
| 표시 | 모양 | 뜻 |
|---|---|---|
| 형광펜 + 밑줄 | 노랑(A) / 하늘(B) / 살구(⚠공개) | 출원 검토가 필요한 구간 |
| 범례 상자 | 첫 슬라이드 "출원검토필요 표시 안내" | 색상별 뜻을 한 번에 안내 (빈자리를 찾아 배치) |
| 배지 | 슬라이드 오른쪽 위 "출원검토필요 N건" | 넘겨 보며 한눈에 파악 |
...
웹 화면의 **[검토 반영]** 탭에 올리면(여러 개 한꺼번에 가능), 교정 내역이 자동으로 쌓여 다음 분석부터 반영됩니다 —
...
| `출원검토필요` 표현 | `app/config.py` | `BADGE_TEXT`(배지) · `LEGEND_TITLE`(범례) · `TAG_TEXT`(선택 문구) |
